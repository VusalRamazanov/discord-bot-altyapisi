...
exports.run = async(client, message, args) => {
  ...
  if (!args[0]) return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`BACON?reklam-engelleme aç\` | Kapatmak İstiyorsanız \`BACON?reklam-engelleme kapat\` Yazabilirsiniz`)
  if (args[0] !== 'aç' && args[0] !== 'kapat') return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`BACON?reklam-engelleme aç\` | Kapatmak İstiyorsanız \`BACON?reklam-engelleme kapat\` Yazabilirsiniz`)
...
exports.help = {
 ...
 description: 'Reklamları engelleyebilirsiniz!',
 usage: 's$$kanal'
};